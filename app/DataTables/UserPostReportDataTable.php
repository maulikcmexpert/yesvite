...
class UserPostReportDataTable extends DataTable
{
            ...
            ->filter(function ($query) {
                if ($this->request->has('search')) {
                    ...
                    $query->where(function ($q) use ($nameParts, $keyword) {
                        ...
                        $q->orwhereHas('event_posts.users', function ($q) use ($keyword) {
                            $q->where('firstname', 'LIKE', "%{$keyword}%")
                              ->orWhere('lastname', 'LIKE', "%{$keyword}%");
                        });
                        ...
                        $q->orWhereHas('event_posts', function ($q) {
                            $q->whereColumn('event_posts.user_id', 'users.id');
                        });
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('username')->title('Username<br><span>(Reported By)</span>')->className('username_By')->orderable(true),
