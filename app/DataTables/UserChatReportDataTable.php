                ...
                if ($this->request->has('search')) {
                    ...
                    $query->where(function ($q) use ($nameParts, $keyword) {
                        if (count($nameParts) === 2) {
                            $q->whereHas('reporter_user', function ($q) use ($nameParts) {
                                $q->where('firstname', 'LIKE', "%{$nameParts[0]}%")
                                  ->where('lastname', 'LIKE', "%{$nameParts[1]}%");
                            })->orWhereHas('to_reporter_user', function ($q) use ($nameParts) {
                                $q->where('firstname', 'LIKE', "%{$nameParts[0]}%")
                                  ->where('lastname', 'LIKE', "%{$nameParts[1]}%");
                            });
                        } else {
                        ...
                        $q->orWhere('report_type', 'LIKE', "%{$keyword}%")
                          ...
                          $q->orWhereHas('reporter_user_id.user', function ($q) use ($keyword) {
                            $q->where('email', 'LIKE', "%{$keyword}%");
                        });
                        $q->orWhereHas('to_be_reported_user_id.user', function ($q) use ($keyword) {
                           $q->where('email', 'LIKE', "%{$keyword}%");
                        });
