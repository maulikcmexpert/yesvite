...

{
    ...
    public function getYesviteContactListPage(Request $request)
    {
        ...
        try {
            ...
            $yesviteRegisteredUser = User::where('id', '!=', $user->id)->where('is_user_phone_contact', '0')->where('user_parent_id',$user->id)->where(function ($query) {
                    ...
                    switch ($val) {
                        ...
                        case 'start_date':
                            if (!empty($request->start_date)) {
                                $udpated[$k][$key] = date('m-d-Y',strtotime($request->start_date));
                            } else {
                        ...
                        case 'end_date':
                            if (!empty($request->end_date)) {
                                $udpated[$k][$key] = date('m-d-Y',strtotime($request->end_date));
                            } else {
