...

{
    ...
    public function createEvent(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['status' => 1, 'event_id' => $eventCreation->id, 'event_name' => $eventData['event_name'], 'message' => "Event Created Successfully", 'purchase_status' => $purchase_status]);
        } catch (QueryException $e) {
            DB::rollBack();
    ...
    public function getEventData(Request $request)
    {
        ...
        try {
            ...
            if ($getEventData != null) {
                ...
                $eventDetail['template_id'] = (!empty($getEventData->template_id) && $getEventData->template_id != NULL) ? (string)$getEventData->template_id : "";
            ...
            if($userSubscription->device_type == 'ios'){
                ...
                foreach ($responce->latest_receipt_info as $key => $value) {
                    if(isset($value->expires_date_ms) && $value->expires_date_ms != null && date('Y-m-d H:i', ($value->expires_date_ms /  1000)) >= date('Y-m-d H:i') ){
                        ...
                        return response()->json(['status' => 1, 'message' => "subscription is active", 'type' => 'Pro-Year','enddate' => $date->format('F d, Y')]); 
                    }
                }
            }else{
                $responce =  $this->set_android_iap($app_id, $product_id, $purchaseToken, 'subscribe');
