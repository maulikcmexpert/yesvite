...
class EventListController extends BaseController
{
    ...
    public function index($selected_date = null, $from_page = null)
    {
        ...
        $totalInvited = EventInvitedUser::where('user_id', $user->id)  // Filter by user_id first
        ...
        if (count($allEvent) != 0) {
            ...
            foreach ($allEvent as $value) {
                ...
                $usercreatedAllPastEventCount = Event::where(['is_draft_save' => '0', 'user_id' => $user->id])->where(function ($query) {
                    $query->where('end_date', '<', date('Y-m-d')) // Past events
                        ->orWhere(function ($q) {
                            $q->where('end_date', '=', date('Y-m-d')) // If event ends today
                            ->whereRaw("STR_TO_DATE(rsvp_start_time, '%h:%i %p') <= STR_TO_DATE(?, '%h:%i %p')", [date('g:i A')]);                    });
        ...
        if (!empty($draftEvents) && count($draftEvents) != 0) {
            foreach ($draftEvents as $value) {
                ...
                $eventDraftDetail['saved_date'] = $value->updated_at;
