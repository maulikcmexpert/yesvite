...

{
    ...
    public function notificationtest()
    {
        ...
        $notificationData = [
            'message' => "IOS data ",
            'type' => 'invite',
            'notification_image' => 'https://plus.unsplash.com/premium_photo-1669324357471-e33e71e3f3d8?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
            'event_id' => '787',
            'post_id' => ""
        ];
        send_notification_FCM_and("c8Gc2h3MSLaffaeDMaZw2p:APA91bFw667qwWQYPa08RPthDO9rHjsoSDqC7scmO9avMs9H1oGXmwbzlZtVxZpC6bl08GZiUMnReflhipxKjFm6h5vVAdsHnpPKBwA2uYRgzOL301p9RfPsyQEL3ZOLZQvUboX4nNfZ", $notificationData);
    }
        ...
        try {
            ...
            if (count($paginatedEvents) != 0) {
                ...
                foreach ($paginatedEvents as $value) {
                    ...
                    if ($value->rsvp_end_time != "" || $value->rsvp_end_time != NULL) {
                        ...
                        if ($checkUserrsvp != null) {
                            ...
                            if ($checkUserrsvp->rsvp_status == '0') {
                                ...
                                if ($value->rsvp_start_time <= strtotime(env('DATE')) && strtotime(env('DATE')) <= $value->rsvp_end_time) {
                        ...
                        $startEventTime = $value->start_date;
                        ...
                        if ($checkUserrsvp != null) {
                            ...
                            if ($checkUserrsvp->rsvp_status == '0') {
                                ...
                                if ($value->rsvp_start_time <= strtotime(env('DATE')) && strtotime(env('DATE')) <= $svrp_end_time) {
            ...
            if ($getEventData != null) {
                ...
                if ($getEventData->event_schedule->isNotEmpty()) {
                    foreach ($getEventData->event_schedule as $eventsScheduleVal) {
                        ...
                        $eventscheduleData["start_time"] = $eventsScheduleVal->start_time;
                        $eventscheduleData["end_time"] = $eventsScheduleVal->end_time;
                        $eventscheduleData["type"] = $eventsScheduleVal->type;
                        $eventDetail['events_schedule_list'][] = $eventscheduleData;
                    }
                }
                ...
                $eventpotluckData =  EventPotluckCategory::with(['event_potluck_category_item' => function ($query) {
    ...
    public function editEvent(Request $request)
    {
        ...
        try {
            ...
            if ($updateEvent != null) {
                ...
                if ($updateEvent->save()) {
                    $getalreadyInviteduser =  EventInvitedUser::where('event_id', $eventData['event_id'])->get()->pluck('user_id');
...

    {
        ...
        try {
            ...
            if ((isset($input['type']) && $input['type'] == '2')) {
                ...
                $total_invitedTo_page = ceil($totalCounts / $this->perPage);
                ...
                $userInvitedEventList = EventInvitedUser::whereHas('event', function ($query) use ($event_date, $end_event_date, $search, $month, $year) {
                    ...
                    $query->where('app_user', '1');
                })->where('user_id', $user->id)->paginate($this->perPage, ['*'], 'page', $page);
            ...
            if ((isset($input['type']) && $input['type'] == '4')) {
                ...
                $total_Hosting_page = ceil($totalCounts / $this->perPage);
                ...
                $hostingEvents =  Event::with(['event_image', 'event_settings', 'user', 'event_schedule'])->where(['is_draft_save' => '0', 'user_id' => $user->id])
                    ->when($event_date || $end_event_date, function ($query) use ($event_date, $end_event_date) {
                        ...
                        return   $query->whereMonth('start_date', $month)->whereYear('start_date', $year);
                    })->orderBy('start_date', 'ASC')->paginate($this->perPage, ['*'], 'page', $page);
                // Make sure to handle the retrieved $userInvitedEventList accordingly
            ...
            if ((isset($input['type']) && $input['type'] == '3')) {
                ...
                $total_pastEvent_page = ceil($totalCounts / $this->perPage);


                $allPastEvents =  collect($allPastEvent)->sortBy('start_date')->forPage($page, $this->perPage);
            ...
            if (!empty($eventList['all']) || !empty($eventList['invited_to']) || !empty($eventList['past_event']) || $eventList['hosting']) {
                return response()->json(['status' => 1, "count" => $totalCounts, 'total_allEvent_page' => $total_allEvent_page, 'total_invitedTo_page' => $total_invitedTo_page, "total_Hosting_page" => $total_Hosting_page, "total_pastEvent_page" => $total_pastEvent_page, 'data' => $eventList, 'message' => "All events"]);
            } else {
                return response()->json(['status' => 0, "count" => $totalCounts, 'total_allEvent_page' => $total_allEvent_page, 'total_invitedTo_page' => $total_invitedTo_page, "total_Hosting_page" => $total_Hosting_page, "total_pastEvent_page" => $total_pastEvent_page, 'data' => $eventList, 'message' => "Events not found"]);
            }
        } catch (QueryException $e) {
            return response()->json(['status' => 0, 'message' => "db error"]);
        } catch (Exception $e) {
...

    {
        ...
        try {
            ...
            foreach ($eventDetail->event_schedule as $key => $value) {
                ...
                $scheduleDetail['activity_title'] = $value->activity_title;
                $scheduleDetail['start_time'] = $value->start_time;
                $scheduleDetail['end_time'] = $value->end_time;
...

    {
        ...
        try {
            ...
            $eventLoginUserStoriesList = EventUserStory::with(['user', 'user_event_story' => function ($query) use ($currentDateTime) {
                $query->where('updated_at', '>', $currentDateTime->subHours(24));
            }])->where(['event_id' => $input['event_id'], 'user_id' => $user->id])->where('updated_at', '>', $currentDateTime->subHours(24))->first();
            ...
            $totalStories =  EventUserStory::with(['user', 'user_event_story' => function ($query) use ($currentDateTime) {
                $query->where('updated_at', '>', $currentDateTime->subHours(24));
            }])->where('event_id', $input['event_id'])->where('user_id', '!=', $user->id)->where('updated_at', '>', $currentDateTime->subHours(24))->count();
            ...
            if (isset($input['type']) && ($input['type'] == '1' || $input['type'] == '0')) {
                ...
                $eventStoriesList = EventUserStory::with(['user', 'user_event_story' => function ($query) use ($currentDateTime) {
                    $query->where('updated_at', '>', $currentDateTime->subHours(24));
                }])->where('event_id', $input['event_id'])->where('user_id', '!=', $user->id)->where('updated_at', '>', $currentDateTime->subHours(24))->paginate($this->perPage, ['*'], 'page', $page);
            } else {
                $total_page_of_stories = ceil($totalStories / $this->perPage);
                $eventStoriesList = EventUserStory::with(['user', 'user_event_story' => function ($query) use ($currentDateTime) {
                    $query->where('updated_at', '>', $currentDateTime->subHours(24));
                }])->where('event_id', $input['event_id'])->where('user_id', '!=', $user->id)->where('updated_at', '>', $currentDateTime->subHours(24))->paginate($this->perPage, ['*'], 'page', "1");
            }
                ...
                $userRsvpStatusList->where(function ($query) use ($selectedFilters) {
                    foreach ($selectedFilters as $filter) {
                        switch ($filter) {
                            ...
                            case 'no_reply':
                                $query->orWhere(function ($qury) {
                                    $qury->where(['rsvp_d' => '0', 'rsvp_status' => NULL]);
                                });
                                break;
                        }
                    }
...

    {
        ...
        try {
            ...
            if ($checkReaction == 0) {
                ...
                $event_post_photo_reaction->unicode = $unicode;
                ...
                $total_counts = EventPostPhotoReaction::where([
                    'event_post_photo_id' => $input['event_post_photo_id']
                ])
                    ->select('reaction', 'unicode', DB::raw('COUNT(*) as count'))
                    ->groupBy('reaction', 'unicode') // Add 'reaction' to the GROUP BY clause
                    ->orderByDesc('count')
                ...
                foreach ($reactionList as $values) {
                    $reactionInfo['id'] = $values->id;

                    $reactionInfo['event_post_id'] = $values->event_post_photo_id;

                    $reactionInfo['reaction'] = $values->reaction;

                    $reactionInfo['user_id'] = $values->user_id;
                    $reactionInfo['username'] = $values->user->firstname . ' ' . $values->user->lastname;
                    $reactionInfo['location'] = ($values->user->city != NULL) ? $values->user->city : "";

                    $reactionInfo['profile'] = (!empty($values->user->profile)) ? asset('public/storage/profile/' . $values->user->profile) : "";
                    $postReaction[] = $reactionInfo;
                }

                $counts = EventPostPhotoReaction::where([

                    'event_post_photo_id' => $input['event_post_photo_id']
                ])->count();
                return response()->json(['status' => 1, 'is_reaction' => 1, 'message' => "Post liked by you", "counts" => $counts, "post_reaction" =>  $postReaction, "reactionList" => $total_counts]);
            } else {


                $message = "";
                $isReaction  = 0;
                $checkReaction = EventPostPhotoReaction::where(['event_post_photo_id' => $input['event_post_photo_id'], 'user_id' => $user->id])->first();
                if ($checkReaction != null) {
                    $unicode = mb_convert_encoding($input['reaction'], 'UTF-32', 'UTF-8');
                    $unicode = strtoupper(bin2hex($unicode));
                    if ($checkReaction->unicode != $unicode) {
                        ...
                        $checkReaction->delete();
                        $removeNotification = Notification::where(['sender_id' => $user->id, 'post_id' => $input['event_post_photo_id'], 'notification_type' => 'like_post'])->first();
                ...
                return response()->json(['status' => 1, 'is_reaction' => 0, 'message' => "Post Disliked by you", "counts" => $counts, "post_reaction" =>  $postReaction, "reactionList" => $total_counts]);
            }
        } catch (QueryException $e) {
...

    {
        ...
        try {
            ...
            $letestComment =  EventPostPhotoComment::with('user')->withcount('post_photo_comment_reaction', 'replies')->where(['event_post_photo_id' => $input['event_post_photo_id'], 'parent_comment_id' => NULL])->orderBy('id', 'DESC')->limit(1)->first();


            $postCommentList = [
                'id' => $letestComment->id,

                'event_post_id' => $letestComment->event_post_photo_id,
                ...
                'comment_total_likes' => $letestComment->post_photo_comment_reaction_count,
    ...
    public function userPostPhotoCommentReply(Request $request)
    {
        ...
        try {
            ...
            $parentCommentId =  $input['parent_comment_id'];
            $mainParentId = (new EventPostPhotoComment())->getMainParentId($parentCommentId);
            ...
            $replyList =   EventPostPhotoComment::with(['user', 'replies' => function ($query) {
                $query->withcount('post_photo_comment_reaction', 'replies')->orderBy('id', 'DESC');
            }])->withcount('post_photo_comment_reaction', 'replies')->where(['id' => $mainParentId, 'event_post_photo_id' => $input['event_post_photo_id']])->orderBy('id', 'DESC')->first();



            $commentInfo['id'] = $replyList->id;

            $commentInfo['event_post_id'] = $replyList->event_post_photo_id;
            ...
            $commentInfo['comment_total_likes'] = $replyList->post_photo_comment_reaction_count;
            ...
            if (!empty($replyList->replies)) {
                ...
                foreach ($replyList->replies as $replyVal) {
                    ...
                    $commentReply['event_post_id'] = $replyVal->event_post_photo_id;
                    ...
                    $commentReply['comment_total_likes'] = $replyVal->post_photo_comment_reaction_count;
                    ...
                    $commentInfo['comment_replies'][] = $commentReply;
                    $replyComment =  EventPostPhotoComment::with(['user'])->withcount('post_photo_comment_reaction', 'replies')->where(['main_parent_comment_id' => $mainParentId, 'event_post_photo_id' => $input['event_post_photo_id'], 'parent_comment_id' => $replyVal->id])->orderBy('id', 'DESC')->get();
                    ...
                    foreach ($replyComment as $childReplyVal) {
                        ...
                        if ($childReplyVal->parent_comment_id != $childReplyVal->main_parent_comment_id) {
                            ...
                            $parentUser = getPhotoParentCommentUserData($childReplyVal->parent_comment_id);
                            $commentChildReply['parent_username'] = $parentUser->user->firstname . ' ' . $parentUser->user->lastname;
                            $commentChildReply['main_comment_id'] = $childReplyVal->main_parent_comment_id;
                            ...
                            $replyChildComment =  EventPostPhotoComment::with(['user'])->withcount('post_photo_comment_reaction', 'replies')->where(['main_parent_comment_id' => $mainParentId, 'event_post_photo_id' => $input['event_post_photo_id'], 'parent_comment_id' => $childReplyVal->id])->orderBy('id', 'DESC')->get();
                            ...
                            foreach ($replyChildComment as $childInReplyVal) {
                                ...
                                if ($childInReplyVal->parent_comment_id != $childInReplyVal->main_parent_comment_id) {
                                    ...
                                    $parentUser = getPhotoParentCommentUserData($childInReplyVal->parent_comment_id);
                                    $commentChildInReply['parent_username'] = $parentUser->user->firstname . ' ' . $parentUser->user->lastname;
                                    $commentChildInReply['main_comment_id'] = $childInReplyVal->main_parent_comment_id;
            ...
            return response()->json(['status' => 1, 'total_comments' => 0, 'data' => $commentInfo, 'message' => "Post commented by you"]);
        } catch (QueryException $e) {
...

    {
        ...
        // ->where('notification_type', '!=', 'upload_post')->where('notification_type', '!=', 'photos');
