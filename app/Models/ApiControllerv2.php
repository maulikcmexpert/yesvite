...

{
        ...
        try {
            ...
            if (!empty($user)) {
                ...
                $profileData = [
                    ...
                    'pending_rsvp_count' => $this->pendingRsvpCount,
    ...
    public function eventListForCalendar()
    {
        try {
            ...
            $eventData = Event::select('event_name', 'start_date')
            ...
            $invitedEventsList = Event::select('event_name', 'start_date')->where('start_date', '>=', $currentMonth)->whereIn('id', $invitedEvents)->where('is_draft_save', '0')->orderBy('start_date', 'ASC');
...

    {
        ...
        try {
            ...
            if ($input['invited_to'] == '1') {
                ...
                if (count($userInvitedEventList) != 0) {
                    ...
                    foreach ($userInvitedEventList as $value) {
                        ...
                        if ($value->event->rsvp_by_date >= date('Y-m-d')) {
            ...
            // Hosting//
            $totalHosting = Event::where(['is_draft_save' => '0', 'user_id' => $user->id])->count();
            ...
            $total_need_rsvp_event_count = EventInvitedUser::whereHas('event', function ($query) {
                $query->where('is_draft_save', '0')->where('rsvp_by_date', '>=', date('Y-m-d'))->where('rsvp_status', NULL);
            })->where('user_id', $user->id)->count();
            ...
            if ($input['need_rsvp_to'] == '1') {
                ...
                $totalCounts += EventInvitedUser::whereHas('event', function ($query) use ($event_date, $end_event_date, $search, $month, $year) {
                    $query->where('is_draft_save', '0')->where('rsvp_by_date', '>=', date('Y-m-d'))
            ...
            if (!empty($eventList)) {
                return response()->json(['status' => 1, "total_invited" => $totalInvited, "total_hosting" => $totalHosting, 'total_past_event_count' => $totalPastEventCount, 'total_need_rsvp_event_count' => $total_need_rsvp_event_count, "count" => $totalCounts, 'total_allEvent_page' => $total_allEvent_page, 'data' => $eventList, 'message' => "All events"]);
            } else {
                return response()->json(['status' => 0, "total_invited" => $totalInvited, "total_hosting" => $totalHosting, 'total_past_event_count' => $totalPastEventCount, 'total_need_rsvp_event_count' => $total_need_rsvp_event_count, "count" => $totalCounts, 'total_allEvent_page' => $total_allEvent_page, 'data' => $eventList, 'message' => "Events not found"]);
            }
        }
