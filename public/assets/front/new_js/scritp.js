...
$(document).on("click", ".mobile-calender-btn-events", function () {
    $(".mobile-menu-wrp").removeClass("active");
    $(".line").removeClass("active");
    // var text = this.innerText;
    var text = this.textContent.trim(); // Use textContent.trim()
...
$(document).on("click", "#detail_calender_view", function () {
    ...
    $('.right-bar-menu-home').addClass("d-none");
    $(".line").removeClass("active");
    // var text = this.innerText;
    var text = this.textContent.trim(); // Use textContent.trim()
    var calendarSvg = `<svg viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
//                     10.0865 1.83331 10.1865 2.44665L10.3332 3.31331" stroke="#0F172A" stroke-width="1.5" stroke-linecap="round"
//                     stroke-linejoin="round"/>
//                 <path d="M12.5664 6.09332L12.1331 12.8067C12.0598 13.8533 11.9998 14.6667 10.1398 14.6667H5.85977C3.99977
//                     14.6667 3.93977 13.8533 3.86644 12.8067L3.43311 6.09332" stroke="#0F172A" stroke-width="1.5" stroke-linecap="round"
//                     stroke-linejoin="round"/>
...
function handleFiles(files, currentFileInput) {
    ...
    Array.from(files).forEach((file) => {
        ...
        fileReader.onload = function (e) {
            ...
            deleteIcon.addEventListener("click", function () {
                const fileNameToDelete = previewDiv.dataset.filename;


                storedFiles = storedFiles.filter(
                    (storedFile) => storedFile.name !== fileNameToDelete
                );
                ...
                console.log("Remaining files:", storedFiles.map(f => f.name));
            });
...
function updateColumnClasses() {
    const imagePreview = document.getElementById("imagePreview");
    const children = Array.from(imagePreview.children);
    children.forEach((previewItem) => {
        previewItem.classList.remove("col-12", "col-6");
