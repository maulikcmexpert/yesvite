...
function savePage4Data() {
    ...
    if ($("#add_co_host").is(":checked")) {
        ...
        if (isCohost == "0" && selected_co_host != "") {
            return;
        }
        eventData.co_host = "";
...
$(document).on("click", ".final_checkout", function () {
    ...
    if (eventData.desgin_selected) {
        $("#eventTempImage").attr("src", base_url + "public/storage/event_images/" + eventData.desgin_selected);
    }

    const sliderContainer = $(".event_images_slider");
    sliderContainer.html(""); // Clear previous images
    ...
    if (eventData.slider_images && eventData.slider_images.length > 0) {
        ...
        // Append new images before initializing Owl Carousel
        sliderContainer.append(imageHtml);

        $(".slider_image_count").text(count + "/3 Photos");

        // **Reinitialize Owl Carousel**
        if ($(".event_images_slider").hasClass("owl-carousel")) {
            $(".event_images_slider").trigger('destroy.owl.carousel').removeClass('owl-carousel owl-loaded');
            $(".event_images_slider").find('.owl-stage-outer').children().unwrap();
        }
