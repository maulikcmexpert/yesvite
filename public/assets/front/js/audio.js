...
export function initializeAudioPlayer(player) {
    ...
    let isMetadataLoaded = false;  // Flag to track metadata loading
    ...
    function updateProgress() {
        if (audio.duration && isMetadataLoaded) {
            const progressPercent = (audio.currentTime / audio.duration) * 100;
            progressBar.style.width = `${progressPercent}%`;
            currentTime.textContent = displayTime(audio.currentTime);
            duration.textContent = `- ${displayTime(audio.duration)}`;
    ...
    function loadSong(songUrl) {
        ...
        audio.addEventListener("loadedmetadata", () => {
            ...
            updateProgress();  // Immediately update progress after metadata loads
        });

        audio.load();
    }

    // Play the audio with smooth progress
    function playSong() {
        if (!isMetadataLoaded) {
            audio.addEventListener("loadedmetadata", () => {
                audio.play();
    ...
    function setProgress(event) {
        const newTime = (event.offsetX / progressRange.offsetWidth) * audio.duration;
