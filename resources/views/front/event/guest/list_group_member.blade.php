    ...
    <div class="users-data">
        <div class="d-flex align-items-start">
            ...
            <div class="text-start">
                ...
                @endif
                @if(isset($data->user->phone_number) && $data->user->phone_number!="")
                <div>
                    <a href="tel">
                        <svg class="me-1" width="14" height="14"
                            viewBox="0 0 14 14" fill="none"
                        ...
                        {{ $data->user->phone_number }}</a>
                </div>
                @endif

            </div>
        </div>
        <div class="d-flex flex-column user_choice_group" data-id="user-{{ $data->user->id }}">
            @if(isset($data->user->email) && $data->user->email!="")
            <div class="right-note d-flex">
                ...
                <input class="form-check-input user_group_member user_choice" type="checkbox"
                    name="add_by_email[]" data-preferby="email" data-id="user-{{$data->user->id}}"data-email="{{ $data->user->email }}"
                    value="{{ $data->user->id }}" checked >
            </div>
            @endif
            @if(isset($data->user->phone_number) && $data->user->phone_number!="")
            <div class="right-note ms-auto"> 
                <input class="form-check-input user_group_member user_choice" type="checkbox"
                name="add_by_mobile[]" data-preferby="phone"data-mobile="{{$data->user->phone_number}}"
                value="{{ $data->user->id }}">
            </div>
            @endif
        </div>
