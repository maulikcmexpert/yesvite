    ...
    @php
        ...
        if (isset($selected_user) && !empty($selected_user)) {
            ...
            if ($foundKey !== false) {
                ...
                $emialAlredy = '';
                $phoneAlredy = '';
                if ($user->id === (int) $selected_user[$key]['sync_id']) {
                    if ($selected_user[$key]['prefer_by'] == 'email') {
                        $email_checked = 'checked';
                        $emialAlredy =
                            isset($selected_user[$key]['isAlready']) && $selected_user[$key]['isAlready'] == '1'
                                ? 'disabled'
                                : '';
                ...
                @if (isset($user->email) && $user->email != '')
                    <div class="right-note d-flex">
                        <input class="form-check-input user_tel-{{ $user->id }} user_choice" type="checkbox"
                            ...
                            data-contact = "1" value="{{ $user->id }}" {{ $email_checked }} {{ $disabled }}
                            {{ $emialAlredy }}>
                    </div>
                @endif
                @if (isset($user->phone_number) && $user->phone_number != '')
                    <div class="right-note ms-auto">
                        <input class="form-check-input user_tel-{{ $user->id }} user_choice" type="checkbox"
                            data-contact = "1" name="mobile[]" data-mobile="{{ $user->phone_number }}"
                            value="{{ $user->id }}" {{ $phone_checked }} {{ $disabled }}
                            {{ $phoneAlredy }}>
                    </div>
                @endif
            </div>
                ...
                @if (isset($user->email) && $user->email != '')
                    <div class="right-note d-flex mb-2">
                        <input class="form-check-input user_group_member user_choice" type="checkbox"
                            name="add_by_email[]" data-preferby="email" data-id="user-{{ $user->id }}"
                            data-email="{{ $user->email }}" data-contact = "1" value="{{ $user->id }}">
                    </div>
                @endif
                @if (isset($user->phone_number) && $user->phone_number != '')
                    <div class="right-note ms-auto">
                        <input class="form-check-input user_group_member user_choice" type="checkbox"
                            name="add_by_mobile[]" data-contact = "1" data-preferby="phone"
                            data-mobile="{{ $user->phone_number }}" value="{{ $user->id }}">
                    </div>
                @endif
            </div>
        @endif
